...
for participant in df_temporal["participant"].unique():
    for label in df_temporal[df_temporal["participant"] == participant][
        "label"
    ].unique():
        subset = df_temporal[
            (df_temporal["participant"] == participant)
            & (df_temporal["label"] == label)
        ].copy()
        for col in predictor_columns:
...
for s in df_temporal["participant"].unique():
    for l in df_temporal[df_temporal["participant"] == s]["label"].unique():
        subset = df_temporal[
            (df_temporal["participant"] == s) & (df_temporal["label"] == l)
        ]
        for col in predictor_columns:
            fig, ax = plt.subplots()
            ...
            ax.plot(
                subset[f"{col}_temp_std_ws_{ws}"], label=f"{col} Std Dev Window {ws}"
            )
            ax.legend()
...
for participant in df_freq["participant"].unique():
    for label in df_freq[df_freq["participant"] == participant]["label"].unique():
        subset = df_freq[
            (df_freq["participant"] == participant) & (df_freq["label"] == label)
        ].copy()
