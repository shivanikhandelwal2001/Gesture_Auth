...
df1 = df[df["label"] == 2]
df1[outlier_columns[:3] + ["label"]].plot.hist(
...
dataset, outliers, X_score = mark_outliers_lof(
...
for col in outlier_columns:
    for label in df["label"].unique():
        dataset = mark_outliers_chauvenet(
            df[df["label"] == label].reset_index(drop=True), col
        )
...
for col in outlier_columns:
    for participant in df["participant"].unique():
        for label in df[df["participant"] == participant]["label"].unique():
            participant_label_data = df[
                (df["participant"] == participant) & (df["label"] == label)
            ]

            dataset = mark_outliers_chauvenet(
                participant_label_data.reset_index(drop=True), col
            )
            dataset.loc[dataset[col + "_outlier"], col] = np.nan
            condition = (outliers_removed_df["participant"] == participant) & (
            ...
            print(
                f"Removed {n_outliers} outliers from {col} for participant {participant} and label {label}"
            )

outliers_removed_df.info()
outliers_removed_df.to_csv(
    "data/interim/02_outliers_removed_chauvnets_new.csv", index=False
)
